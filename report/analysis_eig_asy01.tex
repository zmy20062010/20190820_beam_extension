...
Indeed, we could have higher order ($\geq 2$) expansions, as shown in the appendices (\textcolor{red}{To add some comments}) using an iteration method. Nevertheless, it suffices here to consider up to the second order expansion, and using the following notations:
\begin{equation}
    \left\{\begin{aligned}
        u^{(0)} (z) &= u_0 (z), \\
...
In Figure~\ref{fig:fig_sol_analytic_disp_cmp}, we plot the three approximations $u^{(0)}(z)$, $u^{(1)}(z)$, and $u^{(2)}(z)$, and the closed form solution $u_\delta(z)$.
