...
For different values of $\delta$ and $\sigma$, we calculate the three approximations $u^{(0)}(z)$, $u^{(1)}(z)$, and $u^{(2)}(z)$, and the closed form solution $u_\delta(z)$.


In Figure~\ref{fig:fig_sol_analytic_disp_cmp}.
