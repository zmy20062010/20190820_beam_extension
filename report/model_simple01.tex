...
\begin{table}[!htbp]
    ...
    \hline
       ...
       Length of the extension beam, $l_e$ $(mm)$  & 30 \\
...
In the following, we will investigate the influences of length ratio $\lambda_l$, bending stiffness ratio $\lambda_B$, and line density ratio $\lambda_m$ on the performances of PEHEEs. These three parameters are determined by the length $l_e$, Young's modulus $Y_e$, and volumetric density $\rho_e$ of the extension beam respectively. So we are actually focusing on the influences of this geometric and material properties.

\subsection{Length $l_e$ of the extension beam or length ratio $\lambda_l$}
...
According to Figure~\ref{fig:fig_output_voltage_vs_fr_Rl_laml_all} and Figure~\ref{fig:fig_output_power_vs_fr_Rl_laml_all}, for the given values $\lambda_m$ and $\lambda_B$, frequency response of the PEHEE in the interested frequency range changes in accordance with the extension length $l_e$ and thus the length ratio $\lambda_l$. When $\lambda_l$ is relatively small, say $\lambda_l = 0.2$, as shown in Figure~\ref{fig:fig_output_voltage_vs_fr_Rl_laml_all}(b), frequency response of the PEHEE is almost the same as that of the CPEH case where $\lambda_l = 0.0$. That is to say, there is only one resonant peak in the considered frequency range and the corresponding resonant frequency is around $42\ Hz$ for different values of load resistance $R_l$. Therefore we can conclude that in this situation, the extension beam is playing a negligible role in the motion of the primary beam. They are nearly uncoupled. As a result, energy harvesting performances of the PEHEEs are similar to that of a CPEH. 
...
With the increase of the length ratio $\lambda_l$, say $\lambda_l = 0.4$, $0.6$, or $0.8$, as shown in Figure~\ref{fig:fig_output_voltage_vs_fr_Rl_laml_all}(c), (d), and (e), respectively, there begins to exist an extra resonant and anti-resonant mode in the considered frequency range, compared with that of the case of no elastic extension. An increase of the value of $\lambda_l$ causes the decrease of the resonant and anti-resonant frequencies of the newly occurring modes. What's more, for the cases of $\lambda_l = 0.4$ and $0.6$, the resonant frequency of the previously existing mode, which corresponds to the first resonant mode of a CPEH, is shifted by a small amount. This can be explained as follows. The motion of the PEHEE is actually controlled by the interaction between the primary beam and the extension beam. With the increase of $\lambda_l$, the interaction strengthens from a weak level to a somehow considerable level. When the interaction is weak, the motion of the primary beam and the extension beam is superimposed almost linearly. 

When the interaction is strong enough, the two parts 



With the increase of $\lambda_l$, resonant frequency of the beam extension part approaches that of the primary beam. Thus the two parts interact with each other at a high level. resonant frequency of the whole piezoelectric energy harvester with flexible extension is therefore tuned by the beam extension part. When the resonant frequency of the beam extension is far small than that of the primary beam, which is the case for $\lambda = 0.8$, as shown in Figure~\ref{fig:fig_output_voltage_vs_fr_Rl_laml_all}(d), the two parts have little effect upon each other. 
